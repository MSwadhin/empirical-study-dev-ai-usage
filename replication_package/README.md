...
This folder contains a local replication-package draft for the paper. It is organized around four components:
...
This package intentionally excludes semantic grouping, since that stage is meant to be performed manually by human researchers.
...
- The connected dataset and scripts come from the local `oss-lllm-comments` project workspace.

## Notes

- The compact dataset is stored as JSONL for easier scripting and streaming.
- The copied scripts are preserved as runnable research artifacts; some of them still point to their original default input/output locations, so they should be treated as provenance-preserving script copies rather than a fully re-wired standalone package.
